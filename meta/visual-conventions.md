...
Grand-staff figures use `assets/notation.js`: `grandStaff(svg, flats, sharps)` draws both staves, both clefs, the opening barline, and a key signature, returning the signature as a map; a sharp key passes 0 flats and a count of sharps. `gsNote(svg, x, note, which, opt)` draws one notehead with its ledger lines, taking `sig`, `accX`, `lit`, and `head`; `gsY(note, which)` returns a vertical position; `keySignature(svg, x, flats, sharps)` draws a signature on both staves at x and `cancelSignature(svg, x, flats, sharps)` the naturals that undo one; `stem(host, x, top, bot, up)` draws the stem, at `HEAD_W` and `STEM`, with the notehead glyphs in `HEADS` and their lengths in `DUR`. `which` is `"t"` or `"b"`. Single-staff figures outside part 1 use `staff(svg)` and `chord(g, x, notes, hot, cool)`. Do not write a second copy of any of these.
...
`assets/workbook.js` draws every `svg.answer` and appends the print control to the page's `.controls`. A staff takes `data-staff="single"` for one treble staff, `"grand"` for both, `"figured"` for both with a bass line and its figures already set, or `"numerals"` for both with roman numerals set beneath. A grand or numerals staff takes `data-flats` or `data-sharps` for its key signature. On a figured staff `data-bass`, `data-fig`, `data-key`, `data-num`, `data-flats`, and `data-sharps` hold one entry per example, separated by semicolons: within an example, chords are separated by bars and the figures of one chord by commas.

Every chord on a system takes an equal share of the width, from x = 150 to x = 1440, whichever example it belongs to. An example carries its own key signature, drawn 30 left of its first bass note, preceded by a double bar 18 further left and, where the next key has no accidentals of its own or stands on the other side of the circle, by the naturals that cancel the signature before it. A key label sits 20 right of its item number at y = 52, or at x = 14 on the figure row where the staff holds one example. Figures sit at y = 250 and 17 apart, roman numerals at y = 252. The single staff draws on `viewBox="0 20 1600 190"`, the grand staff on `"0 30 1600 260"`, and the figured and numerals staves on `"0 30 1600 320"`. A blank staff closes at x = 1580 and a staff with content 70 past its last note. The space under the lowest staff is where figures and numerals are written. A staff and the lines belonging to it sit in `.staff-group`, which holds them on one page.
