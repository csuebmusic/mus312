...
A staff of its own with nothing on it is a `"blank"` staff, `data-chords` wide at the same 130 to a chord. A passage written out for the student to name is a `"sonority"` staff: `data-bass` for the bass notes and `data-up` for the voices above each of them, chords separated by bars, drawn in keyboard style at x = 150 in the same box a chord staff takes. Without `data-flats` or `data-sharps` it stands in no key and every accidental it takes is written in; with them it stands in the key its label names and only the notes outside the signature take one. Two of its upper voices a second apart are displaced 0 and 13, and its accidentals all sit in one column, so no two of them may stand within four staff steps of each other. The space under the bass staff is where the chord is named, so the staff carries no answer line.
