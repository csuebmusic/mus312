...
A short example gets a staff to itself rather than a share of a long one, so a key signature never has to be cancelled mid-system. Its bass note stands at x = 150, a second chord 130 further on, the staff closes 70 past the last note, and the `viewBox` takes that width plus 10, at height 336 from y = 14. The box is then held to three quarters of that width in pixels, which sets the notation at the house scale. The item number sits at x = 18, y = 42, at 19px: the G clef reaches 53 units above its baseline at y = 102, so a number set lower runs into it. Boxes sit in `.staff-row`, four to a row and two below 62rem.

A chord written out for the student to name is a `"sonority"` staff: `data-bass` for the bass and `data-up` for the three voices above it, drawn in keyboard style at x = 150 in the same box a chord staff takes. Without `data-flats` or `data-sharps` it stands in no key and every accidental it takes is written in; with them it stands in the key its label names and only the notes outside the signature take one. `data-sig="write"` leaves the signature to the student: the chord is still spelled against it, so the staff shows only what falls outside. Two of its upper voices a second apart are displaced 0 and 13, and its accidentals all sit in one column, so no two of them may stand within four staff steps of each other. The space under the bass staff is where the chord is named, so the staff carries no answer line.
