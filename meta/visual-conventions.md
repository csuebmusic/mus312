...
A short example gets a staff to itself rather than a share of a long one, so a key signature never has to be cancelled mid-system. Its bass note stands at x = 150, a second chord 130 further on, the staff closes 70 past the last note, and the `viewBox` takes that width plus 10, at height 336 from y = 14. The box then caps at three quarters of that width in pixels, which sets the notation at the house scale on screen and lets it shrink below that where the page is narrower, as it is in print. The item number sits at x = 18, y = 42, at 19px: the G clef reaches 53 units above its baseline at y = 102, so a number set lower runs into it. Boxes sit in `.staff-row`, four to a row and two below 62rem, or three to a row with `.staff-row.three` where each holds a passage rather than a chord. The columns are equal fractions rather than the boxes' own width, so a printed page holds the row: printing drops `.staff-row.three` to two across.
