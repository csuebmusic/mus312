...
A metered example on a workbook page is drawn by that page's own script on `svg.example`, at 62 units to the beat, with the meter set once at the head of the system and two examples to a system, the second at x = 830. A voice moving alone carries its own stem and a chord whose voices move together shares one. An augmentation dot is `.dot`, 26 right of its notehead.
