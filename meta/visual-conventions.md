...
`assets/workbook.js` draws every `svg.answer` and appends the print control to the page's `.controls`. A staff takes `data-staff="single"` for one treble staff, `"grand"` for both, `"chord"` for one short example on a staff of its own, `"figured"` for a full-width bass line with its figures set, or `"numerals"` for a full-width staff with roman numerals set beneath. Any of them takes `data-flats` or `data-sharps` for its key signature, `data-bass` for the bass notes, and `data-fig` for the figures, chords separated by bars and the figures of one chord by commas. A chord staff also takes `data-key` and `data-num`.
