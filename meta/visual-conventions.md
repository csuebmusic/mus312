...
A short example gets a staff to itself rather than a share of a long one, so a key signature never has to be cancelled mid-system. Its bass note stands at x = 150, a second chord 130 further on, the staff closes 70 past the last note, and the `viewBox` takes that width plus 10, at height 320 from y = 30. The box is then held to three quarters of that width in pixels, which sets the notation at the house scale. The item number sits at x = 18, y = 52. Boxes sit in `.staff-row`, four to a row and two below 62rem.
...
On a full-width staff the chords take an equal share of the width, from x = 150 to x = 1440. Figures sit at y = 250 and 17 apart, roman numerals at y = 252. The single staff draws on `viewBox="0 20 1600 190"`, the grand staff on `"0 30 1600 260"`, and the figured and numerals staves on `"0 30 1600 320"`. A blank staff closes at x = 1580 and a staff with content 70 past its last note. The space under the lowest staff is where figures and numerals are written. A staff and the lines belonging to it sit in `.staff-group`, which holds them on one page.
