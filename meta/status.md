...
`tools/annotation-guide.html`: Caplin's annotation rules against his Example 2.1, Beethoven's Piano Sonata in F minor, Op. 2 no. 1, i, 1-8. Beethoven's pitches, harmony, and formal boundaries; the rhythm reduced to quarters and halves. One system, eight bars, four flats. Marks carry `data-rule`; pressing a rule lights every mark it governs, and a rule naming a layer lights the layer. Three layer switches, one label switch, and playback. The label switch moves the numerals and the cadence bracket together: Caplin brackets the whole cadential progression, I6 II6 V; this course brackets the pre-dominant to the dominant, ii°6 V, and at an authentic cadence the dominant and its target. The harmonic annotation guide runs as eighteen rule-and-specimen rows.

Bar widths come from content: each onset is given room in proportion to the time before the next, floored at 30 units, and the system scales down if the bars overrun.
